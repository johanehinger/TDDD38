...
template <typename T>
auto get_priority_helper(T&&, long, long, long)
{
  return 4;
}
...
template <typename T>
auto get_priority_helper(T&& t, int, int, long) -> decltype(t.second())
{
  return t.second();
}

template <typename T>
auto get_priority_helper(T&& t, int, long, long) -> decltype(t.third())
{
  return t.third();
}
...
{
    return get_priority_helper(t, 0, 0, 0);
}
