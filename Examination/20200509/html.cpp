...
protected:
    ...
    std::string id;
public:
    Element(std::string const& tag, std::string const& id = {}) : tag{tag}, id{id} {}
    ...
    void print_indent(unsigned indent) const
    {
...
public:
    Content(std::string const& tag, T content, std::string const& id = {}) : 
    ...
    void print(unsigned indent = 0) const override
    {
        print_indent(indent);
        print_tag();
        std::cout << content << "</" << tag << ">" << std::endl;
    }
};

// New representation for type == 1
class Container : public Element
{
private:
    std::vector<Element*> children{};
public:
    Container(std::string const& tag, std::string const& id = {}) : 
        Element(tag, id) {}
...
// New representation for type == 2
class Standalone : public Element
{
public:
    Standalone(std::string tag, std::string const& id = {}) :
        Element(tag, id) {}
...
// New implementation of Hierarchy
class Hierarchy
{
private:
    std::vector<Element*> elements {};
public:
    void insert(Element& element)
    {
        ...
        {
            if (element.get_id() == elements[i]->get_id())
            {
                unique = false;
                break;
            }
        ...
        if (unique)
        {
            ...
            {
                for (Element* child : dynamic_cast<Container*>(&element)->get_children())
                {
                    insert(*child);
                }
            }
    ...
    {
        // Insperation for this came from:
        // http://www.cplusplus.com/reference/algorithm/find_if/
        auto first = std::find_if(std::begin(elements), std::end(elements), [&](Element* element){
            return (element->get_id() == id);
        });

        // find_if will return "last" if none was found.
        // If that is the case then query_id should return nullptr.
        if ( first == std::end(elements))
        {
            return nullptr;
        }
        return *first;
    ...
    std::vector<Element*> query_tag(Element& e, std::string const& tag)
    {
        ...
        // if the element is a Container, query their children as well
        if (dynamic_cast<Container*>(&e))
        {
            for (unsigned i{0}; i < dynamic_cast<Container*>(&e)->get_children().size(); ++i)
            {
                std::vector<Element*> part { query_tag(*dynamic_cast<Container*>(&e)->get_children()[i], tag) };
                for (unsigned j{0}; j < part.size(); ++j)
                {
                    result.push_back(part[j]);
                }
    ...
    std::vector<Element*> query_if(Element& e, Pred predicate)
    {   
        std::vector<Element*> result {};
        if (predicate(e))
        {
        ...
        if (dynamic_cast<Container*>(&e))
        {
            for (unsigned i{0}; i < dynamic_cast<Container*>(&e)->get_children().size(); ++i)
            {
                std::vector<Element*> part { query_if(*dynamic_cast<Container*>(&e)->get_children()[i], predicate) };
                for (unsigned j{0}; j < part.size(); ++j)
                {
                    result.push_back(part[j]);
                }
...
{
    // return e.type == 0;
    if (dynamic_cast<Content<std::string>*>(&e))
    {
        return true;
    }
    return false;
...
int main()
{
    ...
    Container body{"body"};
    Content<std::string> header{"h3", "Title"};
    ...
    Container ul{"ul"};
    Content<std::string> li1 {"li", "Element #1"};
    Content<std::string> li2 {"li", "Element #2"};
    ...
    Content<std::string> p{"p", "This is a paragraph"};
    ...
    std::vector<Element*> divs { hierarchy.query_tag(body, "div") };
    ...
    // std::vector<Element*> contents{hierarchy.query_if(body, only_content)};
    std::vector<Element*> contents{hierarchy.query_if(body, 
    [](Element& element){
        if (dynamic_cast<Content<>*>(&element))
        {
            return true;
        }
        else
